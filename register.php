...
{
    // Process parameters
    $username = $dbh->conn->real_escape_string($_POST["username"]);
    $password = $_POST["password"];
    $crypt_pass = $user->encryptPassword($password);
    $email = $dbh->conn->real_escape_string(trim($_POST["email"]));
    $name = $dbh->conn->real_escape_string(trim($_POST["name"]));
    $address = $dbh->conn->real_escape_string(trim($_POST["address"]));

    // Check condition before validate
    $cond = true;
...
<html lang="en">
  ...
  <head>
	...
	<link href="fonts/vni-souvir/stylesheet.css" rel="stylesheet" type="text/css">
    <!-- Plugin CSS -->
	...
	<script>
		...
		function checkList() {
			...
			// Remove msg_box style if passed all
			if (cond) {
	...
	<style>
		...
		.flex-container {
			...
			font-family: 'vni-souvirbold', 'Open Sans','Helvetica Neue',Arial,sans-serif;
		}
		...
		.form-control:focus {
			border-bottom: solid 2px #f05f40;
  ...
  <body id="page-top">
	...
	<section id="regis" style="padding-top:0px;" class="bg-primary">
		...
		<div class="col-lg-12 col-md-6">
			...
			if ($msg1 <> '')
			{
				echo '<div class="alert alert-danger">' . $msg1 . '</div>';
			}
			?>
			</div>
		</div>
		<div class="col-lg-8 main-center text-center flex-container">
			<div class="bg-orange">
				<form method="post" action="register.php" onsubmit="return checkList(this)">
					...
					<div class="form-group col-lg-6">
						...
						<div class="cols-lg-6">
							<div class="input-group flex-form">
					...
					<div class="row center">
						<div>
							<button type="submit" name="regis" class="btn-register btn btn-outline-primary btn-lg btn-block login-button" onclick="checkList()">Register</button>
						</div>
						<div>
							<a href="login.php"><button type="button" class="btn-register btn btn-outline-primary btn-lg btn-block login-button">Login</button></a>
						</div>
					</div>
					</div>
				</form>
