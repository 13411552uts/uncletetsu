<?php
	...
	if(isset($_POST["btn_submit"])){
		$name = $userObj->conn->real_escape_string(trim($_POST["name"]));
		$price= $userObj->conn->real_escape_string(trim($_POST["price"]));
		$desc = $userObj->conn->real_escape_string(trim($_POST["desc"]));
		if($name && $price && $desc){
			$result = $productObj->insertProducts($name,$desc,$price);
			if ($result) {
				$lastest_product = $productObj->getLastestProductID();
